    ...
    return (
        <Grid2 container justifyContent={"center"} pb={3}>
            ...
            <Grid2 container size={cardSizes} justifyContent={"center"} columnSpacing={3} rowSpacing={2}>
                {[...new Array(data).fill()]?.map((el, ind) => (
                    <Grid2
                        key={`course_${ind}`}
                    ...
                    >
                        {
                            type === "course" ? <CourseCard discountTag={true} /> :
                                type === "classroom" ? <ClassroomCard /> :
                                    type === "instructor" ?
                                        <InstructorCard
                                            profileImage={img1}
            ...
            <Grid2 size={linkSizes} display={"flex"} justifyContent={"flex-end"} pt={2}>
                ...
                <Typography
                    variant="h6"
                    color="base1.default"
                    onClick={() => handleSpecificSearch({ heading, type })}
                    sx={{ cursor: "pointer" }}
                >
                    {linkTxt}
                </Typography>
...
// Export SectionGenerator component
